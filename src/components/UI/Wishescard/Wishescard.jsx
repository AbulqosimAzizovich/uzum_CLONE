...
const Wishescard = ({ item }) => {
  ...
  return (
    <div className="max-w-[232px] h-[456px] font-[Inter] hover:shadow-md duration-500 border">
      <Toast ref={toast} />
      <Link to={`/product/${item?.slugify}`}>
      ...
      <div className="content pt-[11px] pl-2 pr-2">
        ...
        <a
          ...
          href="#"
        >
          {String(Math.floor(item?.price / 3)).replace(
            /\B(?=(\d{3})+(?!\d))/g,
            "."
          )}{" "}
          so'm/oyiga
        ...
        <div className="flex items-center justify-between h-[34px]">
          <div>
            <sub className="text-[11.2px] text-[#757575]">
              <del>
                {String(Math.floor(item?.price + item?.price * 0.12)).replace(
                  /\B(?=(\d{3})+(?!\d))/g,
                  "."
                )}{" "}
                so'm
            ...
            <p className="leading-[16.8px] font-medium text-[14px] ">
              {String(item?.price).replace(/\B(?=(\d{3})+(?!\d))/g, ".")} so'm
            </p>
          </div>
          <div
